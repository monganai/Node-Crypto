...
var Config = function Config(options) {
  ...
  var DD_TRACE_ANALYTICS_ENABLED = coalesce(options.analytics, platform.env('DD_TRACE_ANALYTICS_ENABLED'), platform.env('DD_TRACE_ANALYTICS'), false); // Temporary disabled

  var DD_PROFILING_ENABLED = coalesce( // options.profiling,
  // platform.env('DD_PROFILING_ENABLED'),
  platform.env('DD_EXPERIMENTAL_PROFILING_ENABLED'), false);
  var DD_PROFILING_EXPORTERS = coalesce(platform.env('DD_PROFILING_EXPORTERS'), 'agent');
  var DD_PROFILING_SOURCE_MAP = platform.env('DD_PROFILING_SOURCE_MAP');
  ...
  var DD_TRACE_STARTUP_LOGS = coalesce(options.startupLogs, platform.env('DD_TRACE_STARTUP_LOGS'), true);
  ...
  var DD_TRACE_AGENT_PROTOCOL_VERSION = coalesce(options.protocolVersion, platform.env('DD_TRACE_AGENT_PROTOCOL_VERSION'), '0.4');
  var sampler = options.experimental && options.experimental.sampler || {};
  var ingestion = options.ingestion || {};
  ...
  this.experimental = {
    ...
    peers: options.experimental && options.experimental.distributedTracingOriginAllowlist || options.experimental && options.experimental.distributedTracingOriginWhitelist || [],
...
function extractTags(trace, span) {
  ...
  var internalErrors = span.tracer()._internalErrors;
  ...
  _createClass(Instrumenter, [{
    ...
    value: function wrap(nodules, names, wrapper) {
      ...
      shimmer.massWrap.call(this, nodules, names, function (original, name) {
        ...
        var keys = Reflect.ownKeys(props); // https://github.com/othiym23/shimmer/issues/19
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            ...
            if (_typeof(key) !== 'symbol' || wrapped.hasOwnProperty(key)) continue;
...
var log = {
  ...
  toggle: function toggle(enabled, logLevel, tracer) {
    this._enabled = enabled;
    this._logLevel = _checkLogLevel(logLevel);
    this._tracer = tracer;
    return this;
  },
  _noopSpan: function _noopSpan() {
    if (!this.__noopSpan) {
  ...
  reset: function reset() {
    ...
    this._deprecate = memoize(function (code, message) {
      withNoop(function () {
        return _this._logger.error(message);
      });
      return _this;
    });
  ...
  debug: function debug(message) {
    ...
    if (this._enabled && this._isLogLevelEnabled('debug')) {
      withNoop(function () {
        return _this2._logger.debug(processMsg(message));
      });
    }
  ...
  info: function info(message) {
    ...
    if (this._enabled && this._isLogLevelEnabled('info')) {
      withNoop(function () {
        return _this3._logger.info(processMsg(message));
      });
    }
  ...
  warn: function warn(message) {
    ...
    if (this._enabled && this._isLogLevelEnabled('warn')) {
      withNoop(function () {
        return _this4._logger.warn(processMsg(message));
      });
    }
  ...
  error: function error(err) {
    ...
    if (this._enabled && this._isLogLevelEnabled('error')) {
      ...
      withNoop(function () {
        return _this5._logger.error(err);
      });
    }
...
var NoopSpan = /*#__PURE__*/function (_Span) {
  ...
  function NoopSpan(tracer, parent) {
    ...
    _this._noopTracer = tracer;
    _this._noopContext = _this._createContext(parent);
    return _this;
  }
...
var DatadogSpan = /*#__PURE__*/function (_Span) {
  ...
  function DatadogSpan(tracer, processor, sampler, prioritySampler, fields, debug) {
    var _this;
...
var DatadogTracer = /*#__PURE__*/function (_Tracer) {
  ...
  function DatadogTracer(config) {
    ...
    _this._debug = config.debug;
    _this._internalErrors = config.experimental.internalErrors;
    ...
    _this._enableGetRumData = config.experimental.enableGetRumData;
  ...
  _createClass(DatadogTracer, [{
    ...
    value: function startSpan(name, fields) {
      if (fields) {
        if (fields.references) {
          return _get(_getPrototypeOf(DatadogTracer.prototype), "startSpan", this).call(this, name, fields);
    ...
    value: function _startSpan(name, fields) {
      var reference = getParent(fields.references);
    ...
    key: "_startSpanInternal",
    value: function _startSpanInternal(name) {
...
var RateLimiter = /*#__PURE__*/function () {
  ...
  _createClass(RateLimiter, [{
    ...
    value: function isAllowed() {
      ...
      var curIntervalTokens = this._limiter.tokensThisInterval;
    ...
    value: function effectiveRate() {
      ...
      var allowed = this._prevIntervalTokens + this._limiter.tokensThisInterval;
      var requested = this._prevTokensRequested + this._tokensRequested;
...
var Scope = /*#__PURE__*/function () {
  ...
  _createClass(Scope, [{
    ...
    key: "_wrapEmitter",
    value: function _wrapEmitter(emitter) {
...
function wrapAddListener(addListener) {
  return function addListenerWithTrace(eventName, listener) {
    ...
    var scope = this.__dd_scope;
    var span = this.__dd_span;
    ...
    value: function process(span) {
      ...
      if (trace.started.length === trace.finished.length) {
        ...
        if (spanContext._traceFlags.sampled === false) {
          log.debug(function () {
            return "Dropping trace due to user configured filtering: ".concat(trace.started);
          });
...
var DatadogTracer = /*#__PURE__*/function (_Tracer) {
  ...
  _createClass(DatadogTracer, [{
    ...
    value: function wrap(name, options, fn) {
      ...
      return function () {
        ...
        if (typeof cb === 'function') {
          ...
          return tracer.trace(name, optionsObj, function (span, done) {
    ...
    value: function getRumData() {
      ...
      var span = this.scope().active().context();
      var traceId = span.toTraceId();
      var traceTime = Date.now();
      return "<meta name=\"dd-trace-id\" content=\"".concat(traceId, "\" /><meta name=\"dd-trace-time\" content=\"").concat(traceTime, "\" />");
    }
  }]);
